...
const DEFAULT_MODEL = "glm/glm-5.1";
...
export async function POST(req: NextRequest) {
  ...
  if (!apiKey) {
    return Response.json(
      {
        error:
          "Missing FREETHEAI_API_KEY. Add it to .env.local — see the setup instructions on /ai/freetheai.",
      },
      { status: 500 }
    );
  }
  ...
  const upstream = await fetch(FREETHEAI_URL, {
    method: "POST",
    headers: {
  ...
  if (!upstream.ok || !upstream.body) {
    ...
    return Response.json(
      { error: `FreeTheAi API error (${upstream.status})`, detail: text.slice(0, 500) },
